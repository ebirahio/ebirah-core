...
async function deposit({ currency, amount }) {
  ...
  const noteString = `ebirah-${currency}-${amount}-${netId}-${note}`
  console.log(`Your note: ${noteString}`)
  if (currency === 'bnb') {
    await printETHBalance({ address: tornado._address, name: 'Ebirah' })
    console.log("Before depost", senderAccount)    
    await printETHBalance({ address: senderAccount, name: 'Sender account' })
    const value = isLocalRPC ? ETH_AMOUNT : fromDecimals({ amount, decimals: 18 })
    console.log('Submitting deposit transaction')
    await tornado.methods.deposit(toHex(deposit.commitment)).send({ value, from: senderAccount, gas: 2e6, gasPrice : 20e9 })
    await printETHBalance({ address: tornado._address, name: 'Ebirah' })
    await printETHBalance({ address: senderAccount, name: 'Sender account' })
  } else { // a token
    await printERC20Balance({ address: tornado._address, name: 'Ebirah' })
...
function parseNote(noteString) {
  const noteRegex = /ebirah-(?<currency>\w+)-(?<amount>[\d.]+)-(?<netId>\d+)-0x(?<note>[0-9a-fA-F]{124})/g
...
async function init({ rpc, noteNetId, currency = 'dai', amount = '100' }) {
  ...
  if (inBrowser) {
    ...
    if (PRIVATE_KEY) {
      ...
      //console.log("Private key", PRIVATE_KEY, netId)
  ...
  if (isLocalRPC) {
    ...
    try {
      ...
      console.error('There is no such ebirah instance, check the currency and amount you provide')
      process.exit(1)
    }
  }
